...
(rules generated by `build.py` from the same launch constant every page reads):
waitlist with `?ref=<code>` today, the App Store with `ct=<code>` after launch. Apple's
campaign report is the ledger; the ritual is reading it.

On the 1st of the month:

1. App Store Connect › Analytics › Acquisition › Campaigns › last month. Read
   **First-Time Downloads** per campaign from the dashboard — never from the API's
   detailed report, which is privacy-noised and runs 40%+ low. A campaign only appears
   once it has 5 first-time downloads, and data lags 24–72h, so read on the 1st, not the
   31st.
2. Sheet: creator · code · downloads · × $0.50 · capped at 500 · balance carried. Anything
   under $25 rolls forward.
3. Email each creator their statement. Pay 30 days later by PayPal or Wise (Wise Business
   batch payments work from Mexico; PayPal Payouts needs access requested).
4. Pre-launch, the number to report is waitlist signups per code: Sender › subscribers ›
   filter by the `source` field. Reported, never paid.

Once a quarter, spot-check that active creators disclose the partnership in the content
itself, at the start. FTC §255.1(d) puts that duty on us, and a mention without disclosure
earns nothing under the terms.

Launch day + 1: App Store Connect › Analytics › Acquisition › Campaigns › (+) shows the
`pt=` provider token, which does not exist until the app has ~24h of live downloads. Paste
